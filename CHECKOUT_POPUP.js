...
function getCustomAttributes(radioButton) {
const plan = radioButton.getAttribute('data-planType');
const duration = radioButton.getAttribute('data-planDuration');
return { plan, duration };
}
let plan, duration; 
radioButtons.forEach((radioButton) => {
radioButton.addEventListener('change', () => {
const { plan: selectedPlan, duration: selectedDuration } = getCustomAttributes(radioButton);
plan = selectedPlan; // Update the global variables
duration = selectedDuration; // Update the global variables
});
});
...
const url = `https://drinkprime.in/lpcheckout?plan=${encodeURIComponent(plan)}&duration=${encodeURIComponent(duration)}&name=${encodeURIComponent(nameParam)}&phone=${encodeURIComponent(phoneParam)}&city=${encodeURIComponent(cityParam)}`;
setTimeout(function() {
