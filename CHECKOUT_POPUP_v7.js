...
if (isValid) {
    ...
    function updatePlanAndDuration(event) {
      ...
      function updateUrl() {
        ...
        url = `https://drinkprime.in/lpcheckout?plan=${encodeURIComponent(plan)}&duration=${encodeURIComponent(duration)}&name=${encodeURIComponent(nameParam)}&phone=${encodeURIComponent(phoneParam)}&city=${encodeURIComponent(cityParam)}&coupon=${encodeURIComponent(coupon)}`;
        console.log(url);
      }
